...
def _handle_rss(company, value, count, limit):
    ...
    for entry in fpd.entries:
        ...
        try:
            ...
            print("continuing...")
            continue
        article["title"] = test.convertBijoyToUnicode(content.title)
        article["text"] = test.convertBijoyToUnicode(content.text)
...
def _handle_fallback(company, value, count, limit):
    ...
    for content in paper.articles:
        ...
        if content.publish_date is None:
            ...
            continue
        article = {
            "title": test.convertBijoyToUnicode(content.title),
            "text": test.convertBijoyToUnicode(content.text),
            "link": test.convertBijoyToUnicode(content.url),
            "published": test.convertBijoyToUnicode(content.publish_date.isoformat()),
        }
...
def run(config, limit=4):
    ...
    try:
        with open("scraped_practice.json", "w") as outfile:
...
def main():
    ...
    if len(args) < 2:
        sys.exit("Usage: bengalinewssraper.py bengaliLanguageNewspaper.json")
...
#run "python bengalinewsscrapper.py bengaliLanguageNewspaper.json"